...
for(i in 1:10){
  ...
  SMDNN_model <- SMDNN(trainMat,trainPheno,validMat,validPheno,type = "eps",subp = 2000,localtype = 'CNN',cnnFrame,globalFrame,device_type = "cpu",gpuNum = "max",
                       eval_metric = "mae",num_round = c(10,10),array_batch_size= 30,learning_rate = c(0.01, 0.01),
  ...
  write_csv(res, "D:/Git/data_snp/wheat_res/result.csv", append = TRUE)
  write.table(pred_train, paste0("D:/Git/data_snp/wheat_res/pred_train",i, ".txt"))
  write.table(pred_test, paste0("D:/Git/data_snp/wheat_res/pred_test",i, ".txt"))
}
